...
#### custom parameters #####
model_path = '/home/shenxu/V2S-tensorflow/models/pool_vae_multi_gpu/'
...
class Video_Caption_Generator():
    ...
    # video related computation on GPU1, sentence related computation on GPU0
    ...
    with tf.device("/cpu:4"):
        ...
        with tf.device("/gpu:0"):
            tf_loss, tf_loss_cap, tf_loss_lat, tf_loss_vid, tf_z = model.build_model(train_data, train_video_label, \
                train_caption_id, train_caption_id_1, train_caption_label, drop_sent=drop_sent, drop_video=drop_video, \
                video_weight=video_weight, latent_weight=latent_weight)
        with tf.device("/gpu:0"):
...
#            print 'z:', z[0, :10]
                ...
                if drop_video is 'keep':
                    try:
                        ...
                        print 'epoch', epoch, 'v2s exception'
                if drop_sent == 'keep':
                    try:
                        ...
                        print 'epoch', epoch, 's2s exception'
