...
###### custom parameters #######
model_path = '/data11/shenxu/msrvtt_models/att_vae/'
###### custom parameters #######
...
class Video_Caption_Generator():
    ...
    def build_model(self, video, video_mask, caption, caption_1, caption_mask, drop_sent='keep', 
        ...
        with tf.variable_scope("model") as scope:
            output1 = tf.reduce_mean(image_emb, 0) # b x h 
        ...
        with tf.variable_scope("model") as scope:
            with tf.variable_scope("LSTM3"):
                _, state3 = self.lstm3_dropout(tf.concat(1, [output_semantic, m_init]), state3)
            for i in xrange(self.n_caption_steps):
...
#                e = tf.reduce_sum(e,2) # n x b
                ...
                with tf.variable_scope("LSTM3"):
                    output3, state3 = self.lstm3_dropout(tf.concat(1, [atten, current_embed]), state3) # b x h

                output3_2 = tf.tanh(tf.nn.xw_plus_b(tf.concat(1, [output3,atten,current_embed]),
    ...
    def build_sent_generator(self, video, video_mask):
        ...
        with tf.variable_scope("model") as scope:
            scope.reuse_variables()
            with tf.variable_scope("LSTM3"):
                _, state3 = self.lstm3(tf.concat(1, [output_semantic, m_init]), state3)
...
#                e = tf.reduce_sum(e,2) # n x b
                ...
                with tf.variable_scope("LSTM3") as vs:
                    output3, state3 = self.lstm3( tf.concat(1, [atten, current_embed]), state3 ) # b x h
