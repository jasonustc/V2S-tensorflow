...
### custom parameters ###
model_path = '/home/shenxu/V2S-tensorflow/models/att_rbm/'
### custom parameters ###
...
class Video_Caption_Generator():
    ...
    def build_model(self, video, video_mask, caption, caption_mask,drop_sent='keep', drop_video='keep',
        weight_cap=1., weight_rbm=0.001, weight_vid=1.):
    ...
    def build_sent_generator(self, video, video_mask):
        video_mask = tf.cast(video_mask, tf.float32)
        video_flat = tf.reshape(video, [-1, self.dim_image]) # (b x nv) x d_im
        image_emb = tf.nn.xw_plus_b( video_flat, self.encode_image_W, self.encode_image_b) # (b x nv) x h
        image_emb = tf.reshape(image_emb, [self.batch_size, self.n_video_steps, self.dim_hidden])# b x nv x h
        image_emb = tf.transpose(image_emb, [1,0,2]) # nv x b x h
    ...
    def build_video_generator(self, sent):
        ...
        with tf.variable_scope("model") as scope:
            ...
            for i in range(n_video_step):
                ...
                frame_emb = tf.nn.xw_plus_b(image_prev, self.encode_image_W, self.encode_image_b)
...
def train():
    ...
    with tf.device("/gpu:0"):
        tf_loss, tf_loss_caption, tf_loss_rbm, tf_loss_video = \
            model.build_model(train_data, train_video_label, train_caption_id, train_caption_label)
