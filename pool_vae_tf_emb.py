...
#### custom parameters #####
model_path = '/home/shenxu/V2S-tensorflow/models/pool_sent_emb/'
learning_rate = 0.001
caption_weight = 1.
emb_weight = 0.7
cpu_device = "/cpu:0"
...
class Video_Caption_Generator():
    ...
    def build_model(self, video, caption, caption_mask):
        ...
        output1 = tf.reduce_mean(image_emb, axis=1) # b x h
        # encoding sentence
        word_vec = []
        for i in xrange(self.n_caption_steps):
            current_loc = tf.nn.embedding_lookup(self.loc_matrix, caption[:,i]) # b x n_words
            word_vec.append(current_loc)
        sent_feat = tf.reduce_sum(tf.stack(word_vec), axis=0) # b x n_words
        output2 = tf.matmul(sent_feat, self.sent_emb) # b x h
        loss_emb = tf.reduce_sum(tf.square(output1 - output2)) / batch_size
        ...
        with tf.variable_scope("model") as scope:
            with tf.variable_scope("LSTM3"):
                _, state3 = self.lstm3_dropout(output1, state3) # b x h
        ...
        loss = tf.constant(caption_weight) * loss_caption + tf.constant(emb_weight) * loss_emb
        return loss, loss_caption, loss_emb
...
def train():
    ...
    # graph on the GPU
    with tf.device("/gpu:0"):
        tf_loss, tf_loss_cap, tf_loss_emb \
            = model.build_model(train_data, train_caption_id, train_caption_label)
...
#        print_tensors_in_checkpoint_file(ckpt.model_checkpoint_path, "", True)
    ...
    ##### add summaries ######
    tf.summary.scalar('loss_emb', tf_loss_emb)
...
#        summaries.append(tf.histogram_summary(var.op.name, var))
    ...
    for step in xrange(1, n_steps+1):
        ...
        _, loss_val, loss_cap, loss_emb = sess.run(
                [train_op, tf_loss, tf_loss_cap, tf_loss_emb])
        tStop = time.time()
        print "step:", step, " Loss:", loss_val, "loss_cap:", loss_cap*caption_weight, "loss_emb:", loss_emb*emb_weight
        print "Time Cost:", round(tStop - tStart, 2), "s"
        loss_epoch += loss_val
        loss_epoch_cap += loss_cap
        loss_epoch_emb += loss_emb
...
#        if step % 3 == 0:
            ...
            loss_epoch_emb /= n_epoch_steps
            with tf.device(cpu_device):
                saver.save(sess, os.path.join(model_path, 'model'), global_step=epoch)
            print 'epoch:', epoch, 'loss:', loss_epoch, "loss_cap:", loss_epoch_cap, "loss_lat:",loss_epoch_emb
            loss_epoch = 0
            loss_epoch_cap = 0
            loss_epoch_emb = 0
...
#            n_val_steps = 3
            ...
            if test_v2s:
                try:
                    [pred_sent, gt_sent, id_list, gt_dict, pred_dict, _] = testing_all(sess, 1, ixtoword, val_v2s_tf, val_fname)
