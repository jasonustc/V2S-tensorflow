...
#### custom parameters #####
model_path = '/data11/shenxu/msvd_models/random_scale_frame_tf_s2v/'
learning_rate = 0.001
...
class Video_Caption_Generator():
    ...
    def build_model(self, video_feat, frames, video_mask, caption, caption_1, caption_mask):
        ...
        ######## Decoding Stage ##########
        state3 = (c_init, output_semantic) # 2 x b x h
        state4 = (c_init, output_semantic) # 2 x b x h
        ...
        tf.summary.histogram('decode_image', decode_image)
    ...
    def build_s2v_generator(self, sent, frames):
        ...
        for i in xrange(self.n_caption_steps):
            with tf.device(cpu_device):
                current_loc = tf.nn.embedding_lookup(self.loc_matrix, sent[:,i]) # b x n_words
                word_vec.append(current_loc)
        sent_feat = tf.reduce_sum(tf.stack(word_vec), axis=0) # b x n_words
        output2 = tf.nn.tanh(tf.matmul(sent_feat, self.sent_emb)) # b x h
        ####### Encoding Sentence ##########
...
def train():
    ...
    with tf.device("/gpu:0"):
        ...
        val_s2v_tf,_ = model.build_s2v_generator(val_caption_id_1, val_frame_data)
