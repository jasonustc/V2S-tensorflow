...
class Video_Caption_Generator():
    ...
    def build_model(self, video, encode_video, video_mask, caption, caption_1, caption_mask, drop_sent='keep', 
        drop_video='keep', weight_cap=1., weight_vid=1.):
        ...
        output_semantic = tf.nn.relu(tf.nn.xw_plus_b(input_state, self.embed_sem_W, self.embed_sem_b))
        ######## Semantic Learning Stage ########
    ...
    #            e = tf.reduce_sum(e,2) # n x b
        ...
        with tf.variable_scope("model") as scope:
            ...
            for i in xrange(n_video_steps):
                with tf.variable_scope("LSTM4"):
                    output4, state4 = self.lstm4_dropout(image_emb[i, :, :], state4)
        ...
        loss = weight_cap * loss_caption + weight_vid * loss_video
        return loss, loss_caption, loss_video, output_semantic
    ...
    def build_sent_generator(self, video, encode_video):
        ...
        with tf.variable_scope("model") as scope:
            ...
            for i in xrange(self.n_video_steps):
                with tf.variable_scope("LSTM1"):
                   output1, state1 = self.lstm1_dropout(encode_image_emb[:, i, :], state1) # b x s0
        ####### Encoding Video ##########
    ...
    #            e = tf.reduce_sum(e,2) # n x b
        ...
        generated_words = tf.transpose(tf.stack(generated_words)) # n_caption_step x b
    ...
    def build_video_generator(self, sent):
        ...
        with tf.variable_scope("model") as scope:
            ...
            for i in xrange(self.n_caption_steps):
                with tf.variable_scope("LSTM2"):
                   output2, state2 = self.lstm2_dropout(sent[:, i, :], state2) # b x h
        ####### Encoding Sentence ##########
        ...
        frame_emb = tf.zeros([self.batch_size, self.dim_hidden])
        ...
        with tf.variable_scope("model") as scope:
            ...
            for i in range(n_video_step):
                with tf.variable_scope("LSTM4") as vs:
                    output4, state4 = self.lstm4(frame_emb, state4) # b x h
                    lstm4_variables = [v for v in tf.global_variables() if v.name.startswith(vs.name)]

                frame_prev = tf.nn.xw_plus_b(output4, self.decode_image_W, self.decode_image_b)
                frame_emb = tf.nn.xw_plus_b(frame_prev, self.encode_image_W, self.encode_image_b)
...
def train():
    ...
    with tf.device('/cpu:0'):
        ...
        val_data, val_encode_data, val_video_label, val_fname, val_caption_id_1 = \
            tf.train.batch([val_data, val_encode_data, val_video_label, val_fname, val_caption_id_1], batch_size=batch_size, num_threads=1, capacity=2*batch_size)
