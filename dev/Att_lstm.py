...
### custom parameters ###
model_path = '/home/shenxu/V2S-tensorflow/models/att_lstm/'
### custom parameters ###
...
class Video_Caption_Generator():
    ...
    def build_model(self, video, encode_video, video_mask, caption, caption_mask):
        ...
        state3 = (c_init, m_init) # b x 2 x h
        ...
        with tf.variable_scope("model") as scope:
            with tf.variable_scope("LSTM3"):
                _, state3 = self.lstm3_dropout(tf.concat([output1, output1], 1), state3) # b x h
    ...
    #            e = tf.reduce_sum(e,2) # n x b
                ...
                output3_2 = tf.tanh(tf.nn.xw_plus_b(tf.concat([output3,atten,current_embed], 1), self.embed_nn_Wp, self.embed_nn_bp)) # b x h
    ...
    def build_generator(self, video, encode_video, video_mask):
        video_mask = tf.cast(video_mask, tf.float32)
        ...
        with tf.variable_scope("model") as scope:
            ...
            with tf.variable_scope("LSTM3") as vs:
                _, state3 = self.lstm3( tf.concat([output1, output1], 1), state3) # b x h
            for i in range(self.n_caption_steps):
                e = tf.tanh(tf.matmul(h_prev, self.embed_att_Wa) + image_part) # n x b x h
...
def train():
    ...
    with tf.device("/gpu:0"):
        tf_loss = model.build_model(train_data, train_encode_data, train_video_label, train_caption_id, train_caption_label)
        val_caption_tf, val_lstm3_variables_tf = model.build_generator(val_data, val_encode_data, val_video_label)
...
def test(model_path='models/model-900', video_feat_path=None):
