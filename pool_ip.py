...
class Video_Caption_Generator():
    def __init__(self, dim_image, n_words, dim_hidden, batch_size, n_caption_steps,
        ...
        self.W_semantic = tf.Variable(tf.random_uniform([2*dim_hidden, dim_hidden], -0.01, 0.01), name='W_sem')
        self.b_semantic = tf.Variable(tf.zeros([dim_hidden]), name='b_sem')
...
#    val_data = np.asarray([test_data_folder + 'train000002.h5'])
    ...
    for epoch in range(n_epochs):
        ...
        for current_batch_file_idx in xrange(len(train_data)):
            ...
            tEnd1 = time.time()
            _, loss_val, loss_caption, loss_video = sess.run(
                    [train_op, tf_loss, tf_loss_caption, tf_loss_video],
                    feed_dict={
            ...
            print " Loss_caption:", loss_caption, " Loss_video:", loss_video
